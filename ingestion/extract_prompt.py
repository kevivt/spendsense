"""
Prompt template for turning distilled order email text into structured
...
Shared across platforms (Swiggy food, Instamart, ...) since each
platform-specific distiller normalizes its output to the same intermediate
text shape before it reaches this prompt.
"""

EXTRACTION_SYSTEM_PROMPT = """You are a data extraction engine. You will be given \
the distilled text of an order confirmation email. Extract the order details \
...
- order_date must be in YYYY-MM-DD format. If the input contains a line \
"Order Date: YYYY-MM-DD", that value is already fully resolved - copy it \
exactly, do not recompute or convert it. Otherwise, the email text will \
contain a date like "Jun 26, 7:03 PM" - convert this to full YYYY-MM-DD \
using the year given in the "Reference year" line below.
